...
function capitalizeNames() {
  ...
  let capitalizedNamesArray = []
  items.map((item) => {
    capitalizedNamesArray.push(item.name[0].toLocaleUpperCase() + item.name.slice(1, item.name.length))
  })
  return capitalizedNamesArray;
}
...
function calculateTotalInventory() {
  ...
  return items.reduce((accumalator, currentValue) => ( accumalator + currentValue.inventory), initialValue)
}
...
function calculateAllInventoryPrice() {
  ...
  return items.reduce((previousValue, currentValue) => (previousValue + currentValue.price), 0)
}
...
function logCartItems() {
  ...
  cart.forEach((item, index) => console.log(`Index: ${index}: ${item.name}`))
}
...
const myDiv = document.getElementById("myCart");

myDiv.innerHTML = `
  <h1>My Cart</h1>
  <h2>Items in my cart</h2>
  <ul class="cartItems">
      ${items.map((item) => (`<li>${item.name}</li>`)).join("") /* without .join("") it will return a comma separated list */} 
  </ul>
  <p>The item with id ${itemId} is ${JSON.stringify(findItemById(+itemId), null)}</p>
  <p>We can map over an array and return a new array with the names capitalized like so: ${capitalizeNames()}</p>
  <p>The total inventory of all grocery items is: ${calculateTotalInventory()}</p>
  <p>The total price of all items in inventory is: ${calculateAllInventoryPrice()}</p>
  <p>The price of ${itemToFind} is: $${getItemPriceByName(itemToFind)}</p>
  <p>The items in category ${categoryId} are: ${JSON.stringify(filterItemsByCategoryId(+categoryId), null)}</p>
